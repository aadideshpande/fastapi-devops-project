...
@app.get("/quote/{symbol}",
         response_model=Quote, summary="Get a fake stock quote")
def get_quote(symbol: str):
    ...
    return Quote(symbol=symbol.upper(),
                 price=fake_price, timestamp=date.today())


@app.post("/orders/",
          response_model=OrderResponse, summary="Place a buy/sell order")
def place_order(order: Order):
    # simplistic fill logic: accept if price >= 0
    if order.price <= 0 or order.quantity <= 0:
        raise HTTPException(status_code=400,
                            detail="Invalid price or quantity")
    ORDERS.append(order)
    ...
    if order.side == "buy":
        ...
        if entry:
            total_cost = (entry.avg_price * entry.quantity +
...
@app.get("/portfolio/",
         response_model=List[PortfolioEntry], summary="View portfolio")
def view_portfolio():
    return PORTFOLIO
